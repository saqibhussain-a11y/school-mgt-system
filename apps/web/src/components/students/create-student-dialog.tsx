...
export function CreateStudentDialog({ onCreated }: { onCreated: () => void }) {
  ...
  // Prefilled once per dialog open, editable — the admin can accept it or
  // type a different number (e.g. continuing a legacy series manually).
  // Tracked separately from form.admissionNo so deliberately clearing the
  // field doesn't cause the suggestion to reappear.
  const { data: numberFormat } = useApi<{ suggested: string }>(
    open ? "/api/admission-number-format" : null,
  );
  const [prefilledAdmissionNo, setPrefilledAdmissionNo] = useState(false);
  useEffect(() => {
    if (open && numberFormat && !prefilledAdmissionNo) {
      setForm((f) => ({ ...f, admissionNo: numberFormat.suggested }));
      setPrefilledAdmissionNo(true);
    }
  }, [open, numberFormat, prefilledAdmissionNo]);

  function reset() {
    setForm(EMPTY_FORM);
    setSessionId("");
    setPrefilledAdmissionNo(false);
  }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      toast.success("Student created — no login access yet. Generate credentials from their profile when ready.");
      setOpen(false);
      reset();
      onCreated();
    } catch (err) {
